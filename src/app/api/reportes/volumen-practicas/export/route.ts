...
export async function GET(request: NextRequest) {
  try {
    ...
    const resultado = await ReporteVolumenPracticasService.getVolumenPracticas(
      filtros,
      user.rol,
      user.sedeId || null
    );
    ...
    // Generar CSV
    const csvContent = await ReporteVolumenPracticasService.exportarCSV(resultado.data);

    // Crear nombre del archivo con fecha
    const fecha = new Date().toISOString().split('T')[0];
    const filename = `reporte-volumen-practicas-${fecha}.csv`;
    ...
    console.error('Error en exportación CSV volumen prácticas:', error);
    return NextResponse.json(
